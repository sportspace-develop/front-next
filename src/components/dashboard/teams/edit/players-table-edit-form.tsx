...
const TeamPlayersTableEditForm = React.memo(
  ({ players, onSave, isLoading }: TeamPlayersEditFormProps) => {
    ...
    const handleEditingPlayerChange = React.useCallback(
      (value: PlayerEditFormData | null) => {
        setEditingPlayer(value);
    ...
    const handleAddPlayer = React.useCallback(() => {
      handleEditingPlayerChange(DEFAULT_INITIAL_VALUES_PLAYER);
          ...
          <Table>
            ...
            <TableBody>
              {players.map((player) => (
                <TableRow
                  key={player.id}
                  onClick={() => isMobile && handleEditingPlayerChange(player)}
                ...
                >
                  ...
                  <TableCell sx={{ maxWidth: '36px', p: 0 }}>
                    <IconButton onClick={() => handleEditingPlayerChange(player)} color="primary">
                      <PencilIcon size={20} />
                    </IconButton>
                  </TableCell>
                </TableRow>
